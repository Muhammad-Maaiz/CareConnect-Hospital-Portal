...
# send email template with verification link
@receiver(post_save, sender=Patient)
def send_verification_email(sender, instance, created, **kwargs):
    if created:   # if new patient created
        # Generate link using uuid with url
...
# Send email when user request change password
@receiver(post_save, sender=Patient)
def send_password_reset_email(sender, instance, created, **kwargs):
    ...
    if not created and hasattr(instance, '_send_reset_email'):
        reset_link = f"http://127.0.0.1:8000{reverse('change_password', args=[str(instance.reset_token)])}"
